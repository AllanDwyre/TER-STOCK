...
module.exports = {
  ...
  getTotalCategories: async (req, res) => {
    try {
      ...
      res.status(500).json({
        message:
          "Erreur lors de la récupération du nombre total de categorie: " +
          error.message,
      });
    }
  },
  getTopSellingProduct: async (req, res) => {
    try {
      ...
      res.status(500).json({
        message:
          "Erreur lors de la récupération du top selling product: " +
          error.message,
      });
    }
  },
  ...
  getLowStockProductsCount: async (req, res) => {
    try {
      ...
      res.status(500).json({
        message:
          "Erreur lors de la récupération du nombre de produits en faible stock: " +
          error.message,
      });
    }
  },
